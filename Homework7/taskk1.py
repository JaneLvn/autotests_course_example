...
class Segment:
    ...
    def length(self):
        return round(math.sqrt((self.x2y2[0] - self.x1y1[0]) ** 2 + (self.x2y2[1] - self.x1y1[1]) ** 2), 2)

    def x_axis_intersection(self):
        if (self.x1y1[0] >= 0 >= self.x2y2[0]) or (self.x1y1[0] <= 0 <= self.x2y2[0]):
            return True
        else:
            return False

    def y_axis_intersection(self):
        if (self.x1y1[1] >= 0 >= self.x2y2[1]) or (self.x1y1[1] <= 0 <= self.x2y2[1]):
            return True
        else:
            return False
