...
class PublicTransport:
    ...
    def __init__(self, brand, engine_power, year, colour, max_speed):
        self.brand = brand
    ...
    @property
    def info(self):
        return print(f"Марка: {self.brand}\nМощность двигателя: {self._engine_power}\nГод выпуска: {self.year}\n"
                     f"Цвет: {self.colour}\nМаксимальная скорость: {self.max_speed}")


class Bus(PublicTransport):

    def __init__(self, brand, engine_power, year, colour, max_speed, passengers, park, fare):
        super().__init__(brand, engine_power, year, colour, max_speed)
    ...
    @park.setter
    def park(self, park_number):
        assert 1000 < park_number < 9999
        self.__park = park_number


class Tram(PublicTransport):
    def __init__(self, brand, engine_power, year, colour, max_speed, route, path, fare):
        super().__init__(brand, engine_power, year, colour, max_speed)
        self.__route = route
        self.path = path
        self._fare = fare
